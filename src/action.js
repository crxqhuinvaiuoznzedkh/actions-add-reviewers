    ...
    try {
        ...
        userRemovedReviewers.forEach(reviewer => {
            ...
            if (rev == null) {
                ...
                if (reRequestWhenApproved && debugMode) {
                    core.info(`Approved: Request re-review from ${reviewer}`)
                    finalReviewers.push(reviewer)
                } else if (debugMode) core.info(`Approved: Not requesting re-review from ${reviewer}`)
                } else finalReviewers.push(reviewer)
            }
        )
